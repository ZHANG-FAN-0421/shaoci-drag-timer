...
const state = {
  watchId: null,
  mode: 'idle', // idle, armed, running, stopped
  countdownBusy: false,
  startTime: 0,
  lastFix: null,
  startFix: null,
...
function msFromKmh(k){ return k / 3.6; }
...
function renderTargets(){
  ...
  $('distanceResults').innerHTML = getDistanceTargets().map(v => resultHtml(`${v} m`, `dist_${v}`, true)).join('');
  updateResultsUi();
}

function resultHtml(name, key){
  return `<div class="result" id="res_${key}"><div class="name">${name}</div><div class="value">--</div></div>`;
}

function updateResultsUi(){
  for(const target of speedTargets){
    const el = $(`res_speed_${target}`);
    if(!el) continue;
...
function setGpsBadge(ok, text){
  $('gpsBadge').className = `badge ${ok ? 'good' : 'bad'}`;
...
async function countdown(){
  ...
  for(const t of ['3','2','1','GO']){
    el.textContent = t;
    beep(t === 'GO' ? 1200 : 760, t === 'GO' ? 180 : 90);
...
async function startTest(){
  ...
  state.mode = 'armed';
  setStatus(`GPS 啟動中。倒數後，速度超過 ${state.settings.launchSpeed} km/h 才開始計時。`);
  startWatch();
  await countdown();
  setStatus(`待起跑：超過 ${state.settings.launchSpeed} km/h 自動開始。`);
}
...
function onGeoError(err){
  setGpsBadge(false, 'GPS 錯誤');
  setStatus(`定位失敗：${err.message || '請確認 Safari 定位權限'}`);
}
...
function onPosition(pos){
  ...
  setGpsBadge((fix.accuracy || 999) <= 25, `GPS ±${Math.round(fix.accuracy || 0)}m`);
  ...
  if(state.mode === 'armed' && !state.countdownBusy && speedKmh >= state.settings.launchSpeed){
    beginRun(fix);
  }
...
function addRunningFix(fix){
  ...
  if(segment > 80) segment = 0; // 過濾 GPS 跳點
  ...
  if(state.totalDistance >= state.settings.autoStopDistance){
    stopTest(true);
  }
}
...
function checkTargets(point){
  for(const target of speedTargets){
    ...
    if(point.speedKmh >= target){
      const crossed = interpolateBySpeed(target);
      state.speedResults[target] = {time: fmtTime(crossed.elapsed)};
      beep(1000, 80);
    }
  }
  for(const target of getDistanceTargets()){
    if(state.distanceResults[target]) continue;
    if(point.distance >= target){
      const crossed = interpolateByDistance(target);
      state.distanceResults[target] = {time: fmtTime(crossed.elapsed), trapSpeed: Math.round(crossed.speedKmh)};
      beep(940, 80);
    }
  }
}
...
function interpolateBySpeed(targetKmh){
  ...
  for(let i=1;i<pts.length;i++){
    ...
    if(a.speedKmh <= targetKmh && b.speedKmh >= targetKmh){
      const ratio = (targetKmh - a.speedKmh) / Math.max(0.0001, b.speedKmh - a.speedKmh);
      return mixPoint(a,b,ratio);
    }
  }
...
function interpolateByDistance(targetM){
  ...
  for(let i=1;i<pts.length;i++){
    const a = pts[i-1], b = pts[i];
    if(a.distance <= targetM && b.distance >= targetM){
      const ratio = (targetM - a.distance) / Math.max(0.0001, b.distance - a.distance);
      return mixPoint(a,b,ratio);
    }
  }
  return pts[pts.length-1];
}
function mixPoint(a,b,r){
  r = clamp(r,0,1);
  return {
    elapsed: a.elapsed + (b.elapsed - a.elapsed) * r,
    speedKmh: a.speedKmh + (b.speedKmh - a.speedKmh) * r,
    distance: a.distance + (b.distance - a.distance) * r,
  };
}
...
function stopTest(auto=false){
  ...
  setStatus(auto ? '已達自動結束距離，成績已儲存。' : '測試已停止，成績已儲存。');
  saveRun();
}

function resetRun(clearUi=true){
  state.mode = 'idle';
  state.startTime = 0;
  state.lastFix = null;
  state.startFix = null;
  ...
  state.points = [];
  state.speedResults = {};
  state.distanceResults = {};
  if(state.watchId !== null){ navigator.geolocation.clearWatch(state.watchId); state.watchId = null; }
  $('startBtn').disabled = false;
  $('stopBtn').disabled = true;
  if(clearUi){
    $('speedNow').textContent = '0';
    $('elapsed').textContent = '0.000';
    $('distanceNow').textContent = '0.0';
    $('maxSpeed').textContent = '0';
    setStatus('按開始後，請允許定位。倒數完成後，超過起跑速度才會正式計時。');
  }
  renderTargets();
}
...
function saveRun(){
  ...
  const record = {
    ...
    totalDistance: state.totalDistance.toFixed(1),
    maxSpeed: Math.round(state.maxSpeed),
    speedResults: state.speedResults,
    distanceResults: state.distanceResults,
  };
  const list = getHistory();
  list.unshift(record);
  localStorage.setItem('dt_history', JSON.stringify(list.slice(0,100)));
  renderHistory();
}
function getHistory(){
...
function renderHistory(){
  ...
  $('historyList').innerHTML = list.map(r => {
    const d150 = r.distanceResults?.['150'];
    const d400 = r.distanceResults?.['400'];
    const s100 = r.speedResults?.['100'];
    return `<div class="historyItem"><b>${r.date}</b><p>最高速度：${r.maxSpeed} km/h｜距離：${r.totalDistance} m｜時間：${r.totalTime}s</p><p>0-100：${s100 ? s100.time + 's' : '--'}｜150m：${d150 ? d150.time + 's / ' + d150.trapSpeed + 'km/h' : '--'}｜400m：${d400 ? d400.time + 's / ' + d400.trapSpeed + 'km/h' : '--'}</p></div>`;
  }).join('');
}
